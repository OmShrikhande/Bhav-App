  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      await updateUser({
        ...
        sellerVerified: true,
        brandName: typeof brandName === 'string' ? brandName : Array.isArray(brandName) ? brandName[0] : '', // Update brand name if provided
      });
      
      // Update the Firebase auth store with the new role
      try {
        console.log("Updating Firebase auth store after payment");
        const { firestoreService } = await import('@/services/firestore');
        const userData = await firestoreService.getUserById(userIdStr);
        
        if (userData) {
          console.log("User data found in Firestore after payment:", userData.id, userData.role);
          
          // Import the Firebase auth store
          const { useFirebaseAuthStore } = await import('@/store/firebase-auth-store');
          
          // Update the user in the Firebase auth store
          await useFirebaseAuthStore.getState().updateUserProfile(userData);
          console.log("Firebase auth store updated with new role:", userData.role);
          
          // Force a re-authentication to ensure the new role is applied
          const currentUser = auth.currentUser;
          if (currentUser) {
            console.log("Current Firebase user found, forcing token refresh");
            await currentUser.getIdToken(true);
            console.log("Token refreshed successfully");
          }
        } else {
          console.log("No user data found in Firestore after payment");
        }
      } catch (error) {
        console.error('Error updating Firebase auth store:', error);
      }
      ...
      setTimeout(() => {
        ...
        try {
          // First try to navigate to the splash screen which will handle authentication
          router.push("/splash");
          
          // As a fallback, try to navigate directly to the dashboard after a short delay
          setTimeout(() => {
            ...
            {isSuccess ? (
              ...
              >
                ...
                <Text style={styles.successMessage}>
                  Your seller account has been activated. You will be redirected to the login page.
                </Text>
              </Animated.View>
            ) : (
              <>
                ...
                <View style={styles.paymentMethodContainer}>
                  ...
                  >
                    <CreditCard
                      size={20}
                      color={paymentMethod === 'card' ? "#F3B62B" : "#9e9e9e"}
                  ...
                  >
                    <Smartphone
                      size={20}
                      color={paymentMethod === 'upi' ? "#F3B62B" : "#9e9e9e"}
                      style={styles.paymentMethodIcon}
                    />
                    <Text
                      style={[
                ...
                {paymentMethod === 'card' ? (
                    ...
                    <View style={styles.formContainer}>
                      ...
                      <View style={styles.rowInputs}>
                        <View style={[styles.inputContainer, styles.halfInput]}>
                          <Calendar size={20} color="#F3B62B" style={styles.inputIcon} />
                          <TextInput
                            style={styles.input}
                            placeholder="MM/YY"
                            placeholderTextColor="#9e9e9e"
                  ...
                  <View style={styles.upiContainer}>
                    ...
                    <View style={styles.upiVerificationContainer}>
                      ...
                      <View style={styles.inputContainer}>
                        <Smartphone size={20} color="#F3B62B" style={styles.inputIcon} />
                        <TextInput
                          style={styles.input}
